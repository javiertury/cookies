...
* `httpOnly`: a boolean or the string `signature` indicating whether the cookie is only to be sent over HTTP(S), and not made available to client JavaScript (`true` by default). For a cookie and cookie signature pair, the string `signature` makes the cookie signature unavailable to client JavaScript but doesn't impose that restriction on the cookie.
